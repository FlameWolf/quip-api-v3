...
export default (basePath: Hono) => {
	...
	basePath.delete("/delete/{:name}", validator("param", listInteractParams), listsController.deleteList);
	basePath.get("/{:name}/members", validator("param", listInteractParams), validator("query", listMembersQuery), userController.getListMembers);
	basePath.get("/{:name}/posts", validator("param", listInteractParams), validator("query", listPostsQuery), listsController.getPosts);
};